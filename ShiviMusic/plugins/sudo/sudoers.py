...
from ShiviMusic.utils.database import add_sudo, remove_sudo
from ShiviMusic.utils.decorators.language import language
from ShiviMusic.utils.extraction import extract_user
...
async def sudoers_list(client, message: Message):
    ...
    await message.reply_photo(photo="https://files.catbox.moe/da7vz7.jpg", caption="**» ᴄʜᴇᴄᴋ sᴜᴅᴏ ʟɪsᴛ ʙʏ ɢɪᴠᴇɴ ʙᴇʟᴏᴡ ʙᴜᴛᴛᴏɴ.**\n\n**» ɴᴏᴛᴇ:**  ᴏɴʟʏ sᴜᴅᴏ ᴜsᴇʀs ᴄᴀɴ ᴠɪᴇᴡ. ", reply_markup=reply_markups)
...
async def check_sudo_list(client, callback_query: CallbackQuery):
    ...
    if callback_query.from_user.id not in SUDOERS:
        return await callback_query.answer("𝘽ᴀᴅɴᴀᴍ 𝘿ᴇᴠ ᴋᴀ 𝗟ᴜɴᴅ 𝗟ᴇɢᴀ 😂 \n 𝗦ᴜᴅᴏʟɪsᴛ 𝗗ᴇᴋʜɴᴇ ᴀᴀʏᴀ ᴍᴄ 🤧🖕", show_alert=True)
    else:
    ...
    for user_id in SUDOERS.copy():
        if user_id != OWNER_ID:
            ...
            if removed:
                ...
                count -= 1
    await message.reply_text(f"**» ʀᴇᴍᴏᴠᴇᴅ** `{count}` **ᴜsᴇʀs ғʀᴏᴍ sᴜᴅᴏʟɪsᴛ.**")
